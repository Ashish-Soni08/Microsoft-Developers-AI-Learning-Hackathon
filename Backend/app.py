...
from thesis_rag.thesis_rag_ai_agent import ThesisRagAIAgent
...
@app.post("/ai")
def run_thesis_rag_ai_agent(request: AIRequest):
    """
    Run the Thesis RAG AI agent.
    """
    if request.session_id not in agent_pool:
        agent_pool[request.session_id] = ThesisRagAIAgent(request.session_id)
